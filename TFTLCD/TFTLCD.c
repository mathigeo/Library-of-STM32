...
static uint8_t cmd_fontSize = 16;

/**
 * @brief LCD命令
 * @param cmd 命令符 
 * @param arg 命令参数
 * 	所有颜色参数只支持黑白和三原色-->我懒(^_^)
 *	命令1:fontcolor 	[color]
 *	命令2:fontsize 		[12/16/24]
 *	命令3:clear			[clearColor]
 * 	命令4:display		[on/off]
 *	命令5:setbkcolor 	[color]
 * @retval None
 */
...
int LCD_Print(u8 x,u8 y,const char *formate,...)
{
	...
	va_end(ap);
	LCD_ShowString(x,y,200,16,cmd_fontSize,(uint8_t *)buf);
	return ret;
}
